window.BitTableEditor = {
  mount: function (el, api) {
    ...
    var CHART_META = {
      line: { title: "折线图", hint: "月度趋势 · 改数据即时重绘" },
      bar: { title: "柱状图", hint: "分类对比 · 改数据即时重绘" },
      pie: { title: "饼图", hint: "占比 · 按数值求和归一" },
    };
    ...
    function render() {
      ...
      html +=
        ...
        '<button type="button" data-testid="chart-demo-add" style="height:28px;padding:0 10px;background:#2a2a2a;color:#f5f5f5;border:0;border-radius:4px;cursor:pointer">新增一行</button>' +
        '<button type="button" data-testid="chart-demo-save" style="height:28px;padding:0 10px;background:#3794ff;color:#fff;border:0;border-radius:4px;cursor:pointer">保存</button>' +
        "</div></div>";
      html +=
      ...
      data.rows.forEach(function (row, ri) {
        row = row || {};
        html += '<tr data-testid="chart-demo-row-' + ri + '">';
        fields.forEach(function (f) {
          var key = f.key;
