...
class ChartDemoEditor extends BitTableEditorBase {
  ...
  protected renderLine(pts: Point[], w: number, h: number): string {
    ...
    pts.forEach((p, i) => {
      ...
      path += `${i ? " L " : "M "}${x} ${y}`;
      circles += `<circle cx="${x}" cy="${y}" r="4" fill="#3794ff" stroke="#0a0a0a" stroke-width="2" />`;
      svg += `<text x="${x}" y="${h - 12}" text-anchor="middle" fill="#a3a3a3" font-size="11">${escapeHtml(p.label)}</text>`;
    });
  ...
  protected renderBar(pts: Point[], w: number, h: number): string {
    ...
    pts.forEach((p, i) => {
      ...
      const y = padT + ih - bh;
      svg += `<rect x="${x}" y="${y}" width="${bw}" height="${Math.max(bh, 1)}" fill="${COLORS[i % COLORS.length]}" rx="3" />`;
      svg += `<text x="${x + bw / 2}" y="${h - 14}" text-anchor="middle" fill="#a3a3a3" font-size="11">${escapeHtml(p.label)}</text>`;
    });
    return svg + "</svg>";
  }
  ...
  protected renderPie(pts: Point[], w: number, h: number): string {
    ...
    pts.forEach((p, i) => {
      ...
      svg += `<text x="${legendX + 16}" y="${y}" fill="#d4d4d4" font-size="12">${escapeHtml(p.label)} · ${pct}%</text>`;
    });
    return svg + "</svg>";
  }
